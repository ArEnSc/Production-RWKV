...
class RWKV_RNN_Model():
    ...
    def cpu(self):
        ...
        self.args.FLOAT_MODE = "fp32"  # 7B and up on cpu should use bf16
        os.environ["RWKV_RUN_DEVICE"] = self.args.RUN_DEVICE
        self.model = RWKV_RNN(self.args)
    ...
    def _warp_logits(self, logits: torch.tensor,
        ...
        if repetition_penalty > 0 and len(self.repetition_set) > 0:
            ...
            # converts the input_ids to cuda or cpu
            input_ids = torch.tensor(s).to(torch.device(self.args.RUN_DEVICE))
            # converts the logits to cuda or cpu
            logits = logits.to(torch.device(self.args.RUN_DEVICE))
            ...
            # save to logits using ind and apply the scores there
    ...
    def generate(self,
        ...
        # make sure not to modify the input lol
        context = copy.deepcopy(input_ids)
        ...
        if len(context) > 0:
            ...
            for i in range(len(context)):
                ...
                new_state = self.model.forward(
                    next_token, state, preprocess_only=True)

                if i == len(context)-1:  # last token
                    # get next token from context
                    out_logits, new_state = self.model.forward(
                        next_token, state)
        ...
        for _ in range(max_length):
            ...
            if repetition_penalty > 0:
                # add it as a int so we can use as an index
                self.repetition_set.add(token_id.item())

            context.append(token_id.item())  # array of Ints

            if streaming_callback != None:
                # return token in a int as sequence so it can be decoded
                streaming_callback(token_id.item())
    ...
    def from_pretrained(file_path_or_name: str,
        ...
        if urlparse(url=path).scheme != "" and json_dict != None:
            ...
            if not file in os.listdir():
                # download with wget
                file = wget.download(url=url, out=str(
                    cache_folder_path), bar=bar_progress)
            # model file name
